...
--   3. count/percentage columns deserve the same bounds modules.health has.
--
...
--
--   SELECT * FROM modules p JOIN projects pj ON pj.id = p.project_id
--    WHERE pj.client_id <> p.client_id;              -- fix: SET p.client_id = pj.client_id
--   SELECT * FROM sprints s JOIN components m ON m.id = s.component_id
--    WHERE m.module_id <> s.module_id;             -- fix: SET s.component_id = NULL
--   SELECT * FROM backlog_items b JOIN components m ON m.id = b.component_id
--    WHERE m.module_id <> b.module_id;             -- fix: SET b.component_id = NULL
...
CREATE INDEX modules_current_sprint_id_idx        ON modules (current_sprint_id);
CREATE INDEX generated_reports_sprint_id_idx       ON generated_reports (sprint_id);

-- UNIQUE (module_id, number) already provides a module_id-leading btree.
DROP INDEX sprints_module_id_idx;

-- ------------------------------------------------------- module lineage ---
-- modules.client_id is denormalised. Nothing stopped it from disagreeing with
...
-- destroy the module and its whole subtree. Tie the pair to the project
...
-- reference, and keeping both would run the modules cascade twice per delete
-- under two different ON UPDATE rules.
ALTER TABLE modules
    DROP CONSTRAINT modules_client_id_fkey,
    DROP CONSTRAINT modules_project_id_fkey,
    ADD CONSTRAINT modules_project_client_fkey
        FOREIGN KEY (project_id, client_id) REFERENCES projects (id, client_id)
        ON UPDATE CASCADE ON DELETE CASCADE;

-- ----------------------------------------------------- component lineage ---
-- A sprint or a backlog item may only hang off a Component of its own Component.
-- The column-list form of SET NULL (PostgreSQL 15+) clears just component_id and
-- leaves the NOT NULL module_id alone.
ALTER TABLE components
    ADD CONSTRAINT components_id_module_id_key UNIQUE (id, module_id);

ALTER TABLE sprints
    DROP CONSTRAINT sprints_component_id_fkey,
    ADD CONSTRAINT sprints_component_fkey
        FOREIGN KEY (component_id, module_id) REFERENCES components (id, module_id)
        ON DELETE SET NULL (component_id);

ALTER TABLE backlog_items
    DROP CONSTRAINT backlog_items_component_id_fkey,
    ADD CONSTRAINT backlog_items_component_fkey
        FOREIGN KEY (component_id, module_id) REFERENCES components (id, module_id)
        ON DELETE SET NULL (component_id);

-- ---------------------------------------------------------------- bounds ---
-- modules.health had CHECK (0..100); its siblings did not, so progress:5000
...
ALTER TABLE sprints
    ...
    -- would overflow that expression and break auto-numbering for the module
    -- permanently.
...
ALTER TABLE modules
    ADD CONSTRAINT modules_velocity_check      CHECK (velocity >= 0),
    ADD CONSTRAINT modules_blocked_count_check CHECK (blocked_count >= 0);
...
ALTER TABLE modules
    ADD CONSTRAINT modules_ai_insight_size_check
        CHECK (ai_insight IS NULL OR length(ai_insight::text) <= 65536);
ALTER TABLE roles
...
ALTER TABLE components
    ADD CONSTRAINT components_position_check CHECK (position >= 0);
